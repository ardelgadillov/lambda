...
wxString wxSQLite3Table::GetAsString(int columnIndex)
{
    ...
    {
        wxMessageBox(e.GetMessage() + ";" + wxString::Format(wxT("%i"),m_currentRow) + ";" + wxString::Format(wxT("%i"),columnIndex));
        return("");
    }
}
...
/*************Funciones Andres ****************************/
wxString wxSQLite3Table::ReturnDataString(int row, int col)
{
    SetRow(row);
    return GetString(col);
};

int wxSQLite3Table::ReturnDataInt(int row, int col)
{
    SetRow(row);
    return GetInt(col);
};

double wxSQLite3Table::ReturnDataDouble(int row, int col)
{
    SetRow(row);
...
wxSQLite3Table wxSQLite3Database::GetTable(const char* sql)
{
    ...
    {
        wxMessageBox(e.GetMessage());
        return wxSQLite3Table();
    }
}

/****************Funciones Andres ************/
wxString wxSQLite3Database::ReturnDataString(wxString table, int row, int col)
{
    wxSQLite3Table db_table;
    db_table = GetTable("SELECT * FROM " + table + ";");
    db_table.SetRow(row);
    return db_table.GetString(col);
};

int wxSQLite3Database::ReturnDataInt(wxString table, int row, int col)
{
    wxSQLite3Table db_table;
    db_table = GetTable("SELECT * FROM " + table + ";");
    db_table.SetRow(row);
    return db_table.GetInt(col);
};

double wxSQLite3Database::ReturnDataDouble(wxString table, int row, int col)
{
    wxSQLite3Table db_table;
    db_table = GetTable("SELECT * FROM " + table + ";");
    db_table.SetRow(row);
    return db_table.GetDouble(col);
};
/************************************************/
...
wxSQLite3Database::CreateIntegerCollection(const wxString& collectionName)
{
    ...
    {
        wxMessageBox(e.GetMessage());
        return wxSQLite3IntegerCollection();
    }
}
...
wxSQLite3Database::CreateStringCollection(const wxString& collectionName)
{
    ...
    {
        wxMessageBox(e.GetMessage());
        return wxSQLite3StringCollection();
    }
}
